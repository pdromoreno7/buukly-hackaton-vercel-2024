...
export async function runGeminiText(query: string): Promise<string> {
  try {
    const { text } = await generateText({
      model: google('models/gemini-pro'),
      prompt: `Eres un escritor profesional de libros electrónicos con 20 años de experiencia. Tu estilo es claro, conciso y mantiene un tono formal.
