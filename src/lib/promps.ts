...
Luego, Redacta una lista de 5 capítulos para el libro electrónico titulado como el titulo que creaste y que va a tratar sobre ${query} Proporciona solo los títulos de los capítulos en forma de viñetas. No escribas subcapítulos ni esquemas de los capítulos.
...
export const generateKeyWordToCoverBookPrompt = (title: string) => {
  const PROMPT_GENERATE_TITLE_SHAPTERS_BOOK = `Analiza este título, el cual es de un libro, infiere el tema del libro, 
  y saca solo tres palabra clave de lo que pienses que trate el libro, y traducelas al ingles: ${title}. 
  Crea un json que tenga la siguiete estructura: { keyWordByTitle: "" }. 
